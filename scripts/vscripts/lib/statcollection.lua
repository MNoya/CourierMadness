    ...
    if PlayerResource:IsValidPlayer(playerID) then
        ...
        if IsValidEntity(hero) then
            ...
            table.insert(scoreData, {
                roundsPlayed = GameRules.rounds_played,
                scores = GameRules.scores_list
            })
        end
...
function sendStats(extraFields)
    -- Ensure it is only called once. JK WE SEND IT MULTIPLE TIMES BECAUSE ITS ANOTHER ROUND <3
    ...
    -- Add common stats if they aren't already added (add anyway)
    addStats({
